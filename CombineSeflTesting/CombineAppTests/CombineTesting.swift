...
final class CombineTesting101: CombineTestCase {
    ...
    func testHowPublisherEmitsError() {
        subscribe(publisher: Publishers.Sequence<[String], FileNotFoundError>(sequence: [])) { completion in
            switch completion {
            case .finished:
                print("Finished")
    ...
    func testFetchingBandsFromJson() {
        ...
        subscribe(publisher: (parseJson("bands") as Future<[Band], Error>)) { completion in
            switch completion {
            case .finished:
                print("Finished")
            case .failure(let error):
            ...
            receivedValue = !result.isEmpty
        }

        XCTAssertTrue(receivedValue)
    }

    func testNotFoundFileError() {
    ...
    func testFindSongsByExpectedBandWithSinglePublisher() {
        ...
        subscribe(
            ...
            songsByGunsNRoses = result
        }

        XCTAssertTrue(!songsByGunsNRoses.isEmpty)
        XCTAssertTrue(songsByGunsNRoses.allSatisfy({ $0.bandId == selectedBand.bandId }))
    }
    ...
    func testFindSongsByExpectedBandHavingSeveralPublishers() {
        ...
        subscribe(publisher: matchSongsPublisher) { completion in
            switch completion {
            case .finished:
                print("Finished")
            case .failure(let error):
    ...
    func testPassThroughSubjectTest() {
        ...
        let _ = subscribe(publisher: subject, receiveValue: { receivedValue.append($0) })
